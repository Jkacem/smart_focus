...
\item \textbf{Python}  \textbf{:} Langage de programmation largement utilisé dans le développement web, l'intelligence artificielle et le traitement d'images. On a utilisé Python pour l'ensemble du backend et du pipeline de vision — c'est le langage qui s'imposait naturellement vu l'écosystème IA disponible : FastAPI, MediaPipe, LangChain, tous en Python.\cite{python}.
...
\item \textbf{Dart}  \textbf{:} Langage de programmation développé par Google, optimisé pour la création d'interfaces utilisateur performantes et multiplateformes. Dart est le langage de Flutter — on n'avait pas le choix, mais ça s'est avéré agréable à utiliser une fois pris en main, notamment grâce au typage fort qui a évité pas mal de bugs d'intégration \cite{dart}.
...
\item \textbf{FastAPI}  \textbf{:} Framework Python moderne conçu pour la création d'API REST performantes avec documentation automatique. FastAPI gère tout le backend. On l'a choisi notamment pour la génération automatique de la doc Swagger — pendant les phases de test, pouvoir tester les endpoints directement depuis le navigateur sans Postman nous a fait gagner du temps \cite{fastapi}.
...
\item \textbf{Flutter} \textbf{:} Framework open-source de Google pour le développement d'applications mobiles multiplateformes à partir d'un seul code source. Flutter nous a permis de cibler Android sans avoir à maintenir deux codebases. Le hot reload a beaucoup aidé pendant le développement des interfaces — les modifications s'affichaient instantanément sur l'émulateur \cite{flutter} .
...
\item \textbf{OpenCV} \textbf{:} Bibliothèque open-source de traitement d'images et de vision par ordinateur. OpenCV gère la capture du flux vidéo côté Raspberry Pi. C'est la bibliothèque standard pour ce type de tâche — fiable, légère, et son intégration avec MediaPipe est directe sans conversion de format \cite{opencv}.
...
\item \textbf{MediaPipe} \textbf{:} Framework de Google pour l'analyse en temps réel de flux multimédia. On a retenu MediaPipe pour l'extraction des landmarks faciaux et posturaux. On a testé OpenPose brièvement, mais la latence sur Raspberry Pi était trop élevée. Avec MediaPipe, on tournait à 25 FPS sans GPU — suffisant pour notre cas \cite{mediapipe} .
...
\item \textbf{LangChain} \textbf{:} Framework Python pour le développement d'applications basées sur des modèles de langage. De LangChain, on n'utilise qu'un seul composant : le RecursiveCharacterTextSplitter. Le reste du pipeline RAG est géré directement — on a préféré garder le contrôle plutôt que d'abstraire trop de logique \cite{langchain} .
...
\item \textbf{Groq} \textbf{:} Plateforme d'inférence LLM haute performance. Groq héberge le modèle Llama 3.3 70B qu'on utilise pour le chatbot, les quiz et le planning. La vitesse d'inférence (~400 tokens/s) était le critère principal — c'est ce qui rend le chat fluide sans serveur GPU dédié \cite{groq} .
...
\item \textbf{ChromaDB} \textbf{:} Base de données vectorielle open-source. ChromaDB stocke les vecteurs des documents indexés. On l'a choisi parce qu'il fonctionne entièrement en local sans serveur séparé — un Docker de moins à gérer, et l'API Python est simple \cite{chromadb} .
...
\item \textbf{PostgreSQL} \textbf{:} Système de gestion de base de données relationnelle robuste et open-source. PostgreSQL stocke toutes les données relationnelles du système. On avait envisagé SQLite pour simplifier, mais les besoins en concurrence et la taille des données de monitoring ont vite rendu PostgreSQL nécessaire \cite{postgresql} .
...
\item \textbf{Visual Studio Code} \textbf{:} Éditeur de code source léger et extensible développé par Microsoft. On a travaillé avec VS Code pour l'ensemble du code — backend, vision et Flutter. L'extension Python avec Pylance a vraiment aidé pour naviguer dans un projet aussi éclaté en modules \cite{vscode}  .
...
\item \textbf{Android Studio} \textbf{:} Environnement de développement intégré officiel pour Android. Android Studio a servi uniquement pour l'émulateur Android. On aurait pu s'en passer avec un vrai device, mais pendant les phases de développement rapide, l'émulateur évitait les aller-retours avec le téléphone \cite{androidstudio} .
...
\item \textbf{Git et GitHub} \textbf{:} Système de contrôle de version distribué et plateforme d'hébergement de code. Git et GitHub ont géré le versionnement. Travailler à deux sur des composants aussi différents — matériel, backend, mobile — sans versionning aurait été ingérable. On a eu quelques conflits de merge sur les modèles SQLAlchemy, mais rien de critique \cite{github}  .
...
\item \textbf{Docker} \textbf{:} Plateforme de conteneurisation permettant d'isoler et de déployer des applications. Docker conteneurise FastAPI, PostgreSQL et ChromaDB ensemble. Le vrai avantage, c'est qu'on a évité le classique "ça marche sur ma machine" — l'environnement était identique entre nos deux postes de développement  \cite{docker} .
...
\item \textbf{Draw.io} \textbf{:} Outil de diagrammes en ligne gratuit et open-source.Draw.io a servi pour tous les diagrammes UML du projet. On a choisi l'outil en ligne pour pouvoir collaborer facilement — les fichiers  \cite{drawio} .
...
\item \textbf{Overleaf} \textbf{:} Plateforme \LaTeX{} collaborative en ligne.Overleaf a hébergé la rédaction du mémoire en LaTeX. Collaborer à deux sur un document LaTeX sans outil dédié aurait été compliqué — Overleaf a simplifié la synchronisation et évité les conflits sur les fichiers \cite{overleaf}  .
